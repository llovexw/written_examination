 ...
 *   2.  41位时间戳(毫秒级)，注意，41位时间戳不是存储当前时间的时间戳，而是存储时间戳的差值（当前时间戳 - 开始时间戳)得到的值），
 *       这里的的开始时间戳，一般是我们的id生成器开始使用的时间，由我们程序来指定的（如下下面程序 startTime属性）。41位的时间戳，可以使用69年
 ...
 *  
 * 注意这里进行了小改动:  
 *   1.  Snowflake是5位的datacenter加5位的机器id; 这里变成使用10位的机器id (b)
 *   2.  对系统时间的依赖性非常强，需关闭ntp的时间同步功能。当检测到ntp时间调整后，将会拒绝分配id
 *   3.  如果系统时间被回调，那么会造成id冲突
 *
 * @author luo
 */
public class Snowflake {
    ...
    /**
     * 时间起始标记点，作为基准，一般取系统的最近时间，2022-04-24 16:45:59
     */
    private final long  startTime = 1650789964886L;
    /**
    ...
    public synchronized long nextId() {
        ...
        if (timestamp < lastTimestamp) {
            ...
            System.out.println("clock have promblem");
